...
function AdSlot({ label, height = 90 }: { label: string; height?: number }) {
  return (
    <div style={{ background: "#0f0f0f", border: "1px dashed #2a2a2a", borderRadius: 8, padding: "16px", textAlign: "center" as const, color: "#333", fontSize: 11, margin: "24px 0", minHeight: height, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" as const, gap: 3 }}>
...
function renderMarkdown(content: string): string {
  ...
  return content
    ...
    .replace(/^(?!<[hulbipci])(.*\S.*)$/gm, '<p style="margin:0 0 14px;color:#888880;line-height:1.8;font-size:16px">$1</p>')
    .replace(/\n{2,}/g, "");
}
    ...
    <div style={{ background: T.black, color: T.white, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 16px" }}>
        ...
        <nav style={{ fontSize: 12, color: T.muted2, marginBottom: 20, display: "flex", gap: 5, flexWrap: "wrap" as const }}>
          ...
          {post.category && <><span>/</span><a href={`/blog?category=${post.category}`} style={{ color: T.muted2, textDecoration: "none" }}>{post.category}</a></>}
          <span>/</span>
          <span style={{ color: T.muted }}>{post.title.substring(0, 35)}{post.title.length > 35 ? "..." : ""}</span>
        </nav>

        {post.cover_image && <img src={post.cover_image} alt={post.title} style={{ width: "100%", borderRadius: 12, marginBottom: 24, maxHeight: 380, objectFit: "cover" as const }} />}

        <header style={{ marginBottom: 28 }}>
          {post.category && <a href={`/blog?category=${post.category}`} style={{ fontSize: 11, color: T.gold, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.08em", textDecoration: "none" }}>{post.category}</a>}
          <h1 style={{ fontSize: 30, fontWeight: 700, fontFamily: "Georgia, serif", margin: "8px 0 12px", lineHeight: 1.25 }}>{post.title}</h1>
          {post.excerpt && <p style={{ fontSize: 16, color: T.muted, lineHeight: 1.7, margin: "0 0 16px", fontStyle: "italic" }}>{post.excerpt}</p>}
          ...
          {post.tags?.length > 0 && (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const, marginTop: 12 }}>
              {post.tags.map((t: string) => <a key={t} href={`/blog?tag=${t}`} style={{ padding: "3px 10px", borderRadius: 99, border: `1px solid ${T.border2}`, color: T.muted, fontSize: 11, textDecoration: "none" }}>#{t}</a>)}
            </div>
          )}
        </header>
        ...
        <div style={{ background: T.goldBg, border: `1px solid ${T.gold}30`, borderRadius: 12, padding: "24px 20px", marginTop: 28, textAlign: "center" as const }}>
          ...
          <p style={{ fontSize: 14, color: T.muted, marginBottom: 16, lineHeight: 1.6 }}>Live EUR/USD signals with entry, SL & TP levels.</p>
