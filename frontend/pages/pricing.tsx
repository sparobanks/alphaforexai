...
import { T, Nav, Footer, GoldButton, GhostButton, AuthProvider, useAuth } from "../components/_layout";
  ...
  return (
    <div style={{ background: T.black, color: T.white, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ padding: "72px 32px 32px", textAlign: "center", background: `radial-gradient(ellipse 60% 40% at 50% 0%, rgba(201,168,76,0.06) 0%, transparent 70%)` }}>
        ...
        <p style={{ fontSize: 13, color: T.muted2 }}>Secure payments by Stripe · Cancel anytime</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16, maxWidth: 980, margin: "40px auto 64px", padding: "0 20px" }}>
        {PLANS.map((plan, i) => (
      ...
      {/* Broker note */}
      <div style={{ maxWidth: 980, margin: "-20px auto 40px", padding: "0 20px" }}>
        <div style={{ background: "#120a1e", border: "1px solid #a78bfa30", borderRadius: 12, padding: "20px 24px", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" as const }}>
          <div style={{ fontSize: 28 }}>🏦</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, color: "#c4b5fd", fontSize: 14, marginBottom: 4 }}>Auto-trading broker support</div>
            <p style={{ fontSize: 13, color: T.muted, margin: 0, lineHeight: 1.6 }}>
              Auto-trading currently supports <strong style={{ color: T.white }}>OANDA</strong> (practice & live accounts).
              VIP members can <strong style={{ color: T.white }}>request any broker</strong> from their profile — we add brokers based on user demand.
              Pepperstone, IC Markets, XM, IG Markets and more coming soon.
            </p>
          </div>
          <a href="/register" style={{ padding: "10px 20px", background: "#7c3aed", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" as const }}>
            Get VIP →
          </a>
        </div>
      </div>
