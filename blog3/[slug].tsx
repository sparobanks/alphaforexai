...
// Renders an ad slot from stored HTML code, or a placeholder if empty
function AdSlot({ code, label }: { code?: string; label: string }) {
  if (code) {
    return <div style={{ margin: "24px 0" }} dangerouslySetInnerHTML={{ __html: code }} />;
  }
  return (
    <div style={{ background: "#0f0f0f", border: "1px dashed #2a2a2a", borderRadius: 8, padding: "16px", textAlign: "center" as const, color: "#333", fontSize: 11, margin: "24px 0", minHeight: 80, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" as const, gap: 3 }}>
      <div>Advertisement</div>
      <div style={{ fontSize: 9, color: "#222" }}>{label}</div>
    </div>
  );
}

function renderMarkdownWithAds(content: string, inlineAdCode: string): string {
  if (!content) return "";

  // Replace [AD_SLOT] with actual ad code or placeholder
  const adHtml = inlineAdCode
    ? `<div style="margin:24px 0">${inlineAdCode}</div>`
    : `<div style="background:#0f0f0f;border:1px dashed #2a2a2a;border-radius:8px;padding:16px;text-align:center;color:#333;font-size:11px;margin:24px 0;min-height:80px;display:flex;align-items:center;justify-content:center">Advertisement · Inline</div>`;

  return content
    .replace(/\[AD_SLOT\]/g, adHtml)
...
function PostContent() {
  ...
  useEffect(() => {
    // Fetch site settings for ad codes
  ...
  return (
    <div style={{ background: T.black, color: T.white, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 16px" }}>
        {/* Top banner ad */}
        <AdSlot code={adCodes.top_banner} label="Top Banner — 728x90" />
        ...
        <nav style={{ fontSize: 12, color: T.muted2, marginBottom: 20, display: "flex", gap: 5, flexWrap: "wrap" as const }}>
          ...
          <span>/</span>
          <span style={{ color: T.muted }}>{post.title.substring(0, 35)}{post.title.length > 35 ? "..." : ""}</span>
        </nav>
        ...
        <header style={{ marginBottom: 24 }}>
          {post.category && (
            <a href={`/blog?category=${encodeURIComponent(post.category)}`} style={{ fontSize: 11, color: T.gold, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.08em", textDecoration: "none" }}>
              {post.category}
            </a>
          )}
          ...
          {post.tags?.length > 0 && (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const, marginTop: 12 }}>
              {post.tags.map((t: string) => (
                <a key={t} href={`/blog?tag=${encodeURIComponent(t)}`} style={{ padding: "3px 10px", borderRadius: 99, border: `1px solid ${T.border2}`, color: T.muted, fontSize: 11, textDecoration: "none" }}>#{t}</a>
              ))}
            </div>
          )}
        </header>
        ...
        {/* Before-post ad */}
        <AdSlot code={adCodes.before_post} label="Before Post — 728x90" />

        {/* Content with inline ads */}
        <div style={{ fontSize: 16, lineHeight: 1.8, color: T.muted }}
          dangerouslySetInnerHTML={{ __html: renderMarkdownWithAds(post.content || "", adCodes.inline_ad) }}
        />

        {/* After-post ad */}
        <AdSlot code={adCodes.after_post} label="After Post — 728x90" />

        {/* Share */}
        {showShare && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 24, marginTop: 8 }}>
            <div style={{ fontSize: 13, color: T.muted, marginBottom: 10 }}>Share this article:</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>
              {[
                ...
                <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer"
                  style={{ padding: "7px 14px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 12, color: T.muted, textDecoration: "none" }}>
                  {s.label}
                </a>
              ))}
            </div>
        ...
        <div style={{ background: T.goldBg, border: `1px solid ${T.gold}30`, borderRadius: 12, padding: "24px 20px", marginTop: 28, textAlign: "center" as const }}>
          ...
          <p style={{ fontSize: 14, color: T.muted, marginBottom: 16, lineHeight: 1.6 }}>Live EUR/USD signals with full entry, SL & TP levels. Free to start.</p>
                ...
                <a key={p.id} href={`/blog/${p.slug}`} style={{ textDecoration: "none" }}>
                  <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: "16px" }}>
                    {p.cover_image && !p.cover_image.startsWith("data:") && (
                      <img src={p.cover_image} alt={p.title} style={{ width: "100%", height: 80, objectFit: "cover" as const, borderRadius: 6, marginBottom: 10 }} />
                    )}
